...
import { MOCK_NOTIFICATIONS, MOCK_ROLES } from '../../data/mockData';
import {
  Search,
  Bell,
  Sun,
...
export const Navbar = ({ isCollapsed, onOpenAiModal }) => {
  const { currentRole, switchRole, logout } = useAuth();
  ...
  const handleRoleSelect = (roleId) => {
    ...
    addToast(`Switched active view to ${roleObj?.name}`, 'success');
  };
          ...
          {isProfileOpen && (
            <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-2 z-50 animate-scale-up">
              <div className="px-3 py-2.5 border-b border-slate-100 dark:border-slate-800 mb-2">
                <p className="text-xs font-bold text-slate-900 dark:text-slate-100">Switch Role View</p>
                <p className="text-[11px] text-slate-500 dark:text-slate-400">Instantly test 4 role dashboards</p>
              </div>
              ...
              <div className="space-y-1">
                {MOCK_ROLES.map((r) => (
                  ...
                  >
                    ...
                    {currentRole.id === r.id && <CheckCircle2 className="w-4 h-4 text-indigo-500" />}
                  </button>
                ))}
              </div>
