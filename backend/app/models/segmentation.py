...
class CustomerSegmentAssignment(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    first_purchase: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    ...
    active_days: Mapped[int | None] = mapped_column()
    active_months: Mapped[int | None] = mapped_column()
    product_variety: Mapped[int | None] = mapped_column()
    tenure_days: Mapped[int | None] = mapped_column()
    average_days_between_orders: Mapped[Decimal | None] = mapped_column(Numeric(12, 4))
    return_order_count: Mapped[int | None] = mapped_column()
    returned_value: Mapped[Decimal | None] = mapped_column(Numeric(14, 2))
    return_rate: Mapped[Decimal | None] = mapped_column(Numeric(8, 6))
