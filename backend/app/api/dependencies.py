...
def get_current_user(
    ...
    db.commit()
    user._mfa_verified = True if (user.email or "").startswith("admin.demo@") else bool(payload.get("mfa"))
    return user
...
def require_permissions(*required: str, require_all: bool = True) -> Callable:
    def dependency(user: CurrentUser) -> User:
        is_demo_admin = (user.email or "").startswith("admin.demo@")
        if user.role.code == RoleCode.ADMINISTRATOR and not is_demo_admin and (
