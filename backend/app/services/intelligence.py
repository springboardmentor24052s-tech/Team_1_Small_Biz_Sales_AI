...
def train_tenant_intelligence(db: Session, tenant_id: UUID) -> dict[str, Any]:
    ...
    churn_run = train_tenant_churn_model(db, tenant_id)
    modules["churn"] = {
        "status": "trained" if churn_run else "not_ready",
        "model_version": churn_run.model_version if churn_run else None,
        "accuracy": churn_run.accuracy if churn_run else None,
    }

    rec_run = train_tenant_recommendations(db, tenant_id)
    modules["recommendations"] = {
        "status": "trained" if rec_run else "not_ready",
        "model_version": rec_run.model_version if rec_run else None,
        "rule_count": rec_run.rule_count if rec_run else 0,
    }

    anom_run = train_and_scan_anomalies(db, tenant_id)
    modules["anomalies"] = {
        "status": "trained" if anom_run else "not_ready",
        "model_version": anom_run.model_version if anom_run else None,
        "detection_rate": anom_run.detection_rate if anom_run else None,
    }
