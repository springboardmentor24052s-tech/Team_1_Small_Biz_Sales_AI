...
import { ShoppingBag, Search, Plus, FileText, Download, Pencil, Ban, PackagePlus, Trash2, Sparkles } from 'lucide-react';
...
export const SalesModule = () => {
  ...
  const filteredDeals = deals.filter((deal) => {
    ...
    if (!term) return true;
    const ref = (deal.displayReference || '').toLowerCase();
    const sys = (deal.source_system || '').toLowerCase();
    const st = (deal.status || '').toLowerCase();
    const pm = (deal.payment_method || '').toLowerCase();
    const amt = String(deal.total_amount || '');
    const dispAmt = (deal.amount || '').toLowerCase();
    const notes = (deal.notes || '').toLowerCase();
    return ref.includes(term) || sys.includes(term) || st.includes(term) || pm.includes(term) || amt.includes(term) || dispAmt.includes(term) || notes.includes(term);
  });
  ...
  const fetchCrossSell = async (productId) => {
    if (!productId) {
      setRecommendations([]);
      return;
    }
    try {
      const res = await api(`/recommendations/frequently-bought-together?product_id=${productId}&limit=3`);
